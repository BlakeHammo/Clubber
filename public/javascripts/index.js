...
function login(event) {
    /* use the event object to prevent the form executing it's
    default behaviour (POST to / since no "action" property is specifed in the html)
    this way, if the login is unsucessful, the page will just
    remain the same so the user can try again if the login is successful,
    they will be redirect to feed.html by the window.location.href below. */
...
function signup() {
    return new Promise(function(resolve, reject) {
        ...
        xhttp.onreadystatechange = function() {
            if(xhttp.readyState === 4 && xhttp.status === 200) {
                resolve(true);
                // if everything is fine, return promise (true) to the awaiting submitSignupForm()
            } else if(xhttp.readyState === 4 && xhttp.status === 401) {
                resolve(false);
                // else return promise (false) to to the awaiting submitSignupForm()
            } else if(xhttp.readyState === 4 && xhttp.status === 422) {
                // 400 error if password too short (< 12 characters)
...
// await signup() method's promise, if it's successful, then submit the
// form and redirect user to login
